...
static int
procgen(Chan *c, char *name, Dirtab *tab, int _, int s, Dir *dp)
{
	Qid qid;
	Proc *p;
	char *ename;
...
static void
_proctrace(Proc* p, int etype, vlong ts)
{
	Traceevent *te;
...
proctext(Chan *c, Proc *p)
{
	...
	if(incref(&tc->ref) == 1 || (tc->flag&COPEN) == 0 || tc->mode!=OREAD) {
		cclose(tc);
		error(Eprocdied);
	}
