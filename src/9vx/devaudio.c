...
resetlevel(int dev)
{
	...
	for(i=0; volumes[i].name; i++)
		audiodevsetvol(dev, i, volumes[i].ilval, volumes[i].irval);
}
...
audioopen(Chan *c, int omode)
{
	...
	case Qaudio:
		...
		aqlock(&audio[c->dev]);
		if(waserror()){
			aqunlock(&audio[c->dev]);
			nexterror();
		}
		if(audio[c->dev].amode != Aclosed)
			error(Einuse);
		audiodevopen(c->dev);
		audio[c->dev].amode = amode;
		poperror();
		aqunlock(&audio[c->dev]);
		break;
	}
...
audioread(Chan *c, void *v, long n, vlong off)
{
	...
	case Qvolume:
		...
		for(m=0; volumes[m].name; m++){
			if(waserror())
				continue;
			audiodevgetvol(c->dev, m, &lov, &rov);
			poperror();
			liv = lov;
			riv = rov;
		...
		for(i = 0; i < cb->nf; i++){
			...
			if(cb->f[i][0] >= '0' && cb->f[i][0] <= '9') {
				...
				if(!out)
					goto cont0;
				if(left && right)
					audiodevsetvol(c->dev, v, m, m);
				else if(left)
					audiodevsetvol(c->dev, v, m, -1);
				else if(right)
					audiodevsetvol(c->dev, v, -1, m);
				goto cont0;
			}
