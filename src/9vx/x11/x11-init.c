...
_xattach(char *label, char *winsize)
{
	...
	*/
	...
	_x.kmcon = XOpenDisplay(NULL);
	if(_x.kmcon == nil)
		panic("XOpenDisplay kmcon");

	_x.snarfcon = XOpenDisplay(NULL);
	if(_x.snarfcon == nil)
		panic("XOpenDisplay snarfcon");
	...
	termreplacescreenimage(_x.screenimage);
...
flushmemscreen(Rectangle r)
{
	...
	if(up && !up->nlocks.ref && _x.nextscreenpm != _x.screenpm){
		qlock(&_x.screenlock);
		XSync(_x.display, False);
...
_xconfigure(XEvent *e)
{
	...
	if(!fullscreen){
		int rx, ry;
		XWindow w;
		if(XTranslateCoordinates(_x.kmcon, _x.drawable, DefaultRootWindow(_x.display), 0, 0, &rx, &ry, &w))
