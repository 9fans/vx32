...
_xtoplan9mouse(XEvent *e, Mouse *m)
{
	...
	if(_x.putsnarf != _x.assertsnarf){
		...
		XSetSelectionOwner(_x.kmcon, XA_PRIMARY, _x.drawable, CurrentTime);
		if(_x.clipboard != None)
			XSetSelectionOwner(_x.kmcon, _x.clipboard, _x.drawable, CurrentTime);
		XFlush(_x.kmcon);
	}
...
__xputsnarf(char *data)
{
	...
	e.state = ~0;
	e.button = ~0;
	XSendEvent(_x.snarfcon, _x.drawable, True, ButtonPressMask, (XEvent*)&e);
	XFlush(_x.snarfcon);
	qunlock(&clip.lk);
}
...
if(0) fprint(2, "xselect target=%d requestor=%d property=%d selection=%d\n",
	...
	if(xe->target == _x.targets){
		...
		XChangeProperty(_x.kmcon, xe->requestor, xe->property, xe->target,
			8*sizeof(a[0]), PropModeReplace, (uchar*)a, nelem(a));
	}else if(xe->target == XA_STRING 
	|| xe->target == _x.utf8string 
	|| xe->target == _x.text 
	|| xe->target == _x.compoundtext
	|| ((name = XGetAtomName(_x.kmcon, xe->target)) && strcmp(name, "text/plain;charset=UTF-8") == 0)){
		...
		if(strcmp(name, "TIMESTAMP") != 0)
			fprint(2, "9vx: cannot handle selection request for '%s' (%d)\n", name, (int)xe->target);
	...
	XSendEvent(_x.kmcon, xe->requestor, False, 0, &r);
	XFlush(_x.kmcon);
	return 0;
}
